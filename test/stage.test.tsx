...
describe('<Stage />', () => {
  it('renders without crashing', () => {
    const result = Enzyme.shallow(
      <Stage lvl={1} dificult={"easy"} timeoutAnswer={() => {}} handleClick={(e: any, time: number) => {}}/>
    )
  ...
  it ('No render if lvl 0', () => {
    const result = Enzyme.shallow(
      <Stage lvl={0} dificult={"easy"} timeoutAnswer={() => {}} handleClick={(e: any, time: number) => {}}/>
    )
  ...
  it ('No render if lvl 11', () => {
    const result = Enzyme.shallow(
      <Stage lvl={11} dificult={"easy"} timeoutAnswer={() => {}} handleClick={(e: any, time: number) => {}}/>
    )
    expect(result.find(Timer).length).toEqual(0);
  })
  ...
  it ('render if lvl between 1 and 10', () => {
    for (let i = 1; i <= 10; i++) {
      const result = Enzyme.shallow(
        <Stage lvl={i} dificult={"easy"} timeoutAnswer={() => {}} handleClick={(e: any, time: number) => {}}/>
      )
  ...
  it ('allows us to set props', () => {
    ...
    expect(wrapper.props().dificult).toEqual("easy")
    wrapper.setProps({dificult: "normal"})
    expect(wrapper.props().dificult).toEqual("normal")
  })
